...
class ListGoalPeriodController{
  async handle(req: Request, res: Response){
    const { id, amount, category_id, period_id, amount_compare} = req.body;
    ...
    const goalPeriod = await new ListGoalPeriodService().execute({ id, amount,category_id, period_id, amount_compare})
