...
class UpdateExpenseController{
  async handle(req: Request, res: Response){
    const { id, date, description, value, category_id, period_id} = req.body;
    ...
    const expense = await updateExpenseService.execute({ id, date, description, value, category_id, period_id,  updated_by});
