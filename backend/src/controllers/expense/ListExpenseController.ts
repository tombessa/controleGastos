...
class ListExpenseController{
  async handle(req: Request, res: Response){
    const { id, date, description, value, category_id, period_id, date_compare, date_ini, date_fim} = req.body;
    ...
    const expense = await new ListExpenseService().execute({ id, date, description, value, category_id, period_id, date_compare,date_ini, date_fim})
